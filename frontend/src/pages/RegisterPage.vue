  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-100 via-sky-100 to-cyan-100 px-4 py-8">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-5xl w-full items-center">
      ...
      <div class="rounded-3xl border border-white/80 bg-white/90 shadow-xl p-8 md:p-10 backdrop-blur">
        <h1 class="tl-title text-4xl font-bold text-slate-900 mb-2">Join TravelLink</h1>
        <p class="text-slate-600 mb-6">Create your account to start sharing travel plans.</p>
        ...
        <form @submit.prevent="handleRegister" class="space-y-4 mb-6">
          ...
          <button type="submit" class="w-full px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-sky-600 text-white rounded-xl text-sm font-semibold cursor-pointer transition-all hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed" :disabled="authStore.isLoading">
        ...
        <router-link to="/login" class="block w-full px-4 py-2.5 bg-slate-100 text-slate-900 rounded-xl text-sm font-medium text-center cursor-pointer transition-all hover:bg-slate-200 border border-slate-300">
      ...
      <div class="hidden lg:flex flex-col items-center justify-center">
        <svg viewBox="0 0 200 200" fill="none" stroke="currentColor" class="w-40 h-40 mb-6 text-emerald-800">
          <circle cx="100" cy="100" r="80" stroke-width="2" />
          <path d="M 75 80 Q 100 60 125 80" />
          <circle cx="100" cy="100" r="20" />
          <path d="M 75 130 L 100 120 L 125 130" />
        </svg>
        <p class="text-slate-700 text-lg text-center leading-relaxed max-w-md">Start your journey and inspire others with your curated travel routes.</p>
      </div>
    </div>
  </div>
</template>
...
async function handleRegister(): Promise<void> {
  ...
  try {
    ...
    uiStore.showError(authStore.error || 'Registration failed');
  }
}
</script>
